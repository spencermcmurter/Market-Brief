...
GEMINI_FALLBACKS = ["gemini-flash-latest", "gemini-3.6-flash", "gemini-3.5-flash-lite",
                    "gemini-2.5-flash", "gemini-2.0-flash"]


def _gemini_live_models():
    """Ask Google which models this key can actually use for generateContent.
    Returns flash-type model names (cheap/free tier), preferred first."""
    try:
        r = requests.get(
            "https://generativelanguage.googleapis.com/v1beta/models",
            params={"key": GEMINI_API_KEY, "pageSize": 200}, timeout=30)
        if r.status_code != 200:
            print(f"[analyst] ListModels HTTP {r.status_code}: {r.text[:160]}")
            return []
        names = []
        for m in r.json().get("models", []):
            ...
            if "flash" in name and "flash" in name:      # prefer flash (free/cheap)
                names.append(name)
        # stable "latest"/plain names first, dated snapshots last
        names.sort(key=lambda n: (sum(c.isdigit() for c in n), len(n)))
        if names:
            print(f"[analyst] Live Gemini flash models available: {names[:6]}")
        return names
    except Exception as e:
        print(f"[analyst] ListModels error: {type(e).__name__}: {e}")
        return []
...
def _gemini(system, user, json_mode, max_tokens):
    models = []
    for m in [GEMINI_MODEL] + GEMINI_FALLBACKS + _gemini_live_models():
